angular.module('app').component('collection', {
	bindings: { organizationId: '<', collectionTypeId: '<', collectionId: '<'},
 	...
 	controller: ["$scope", "$itemService", "$collectionTypes", "$mdDialog", "$rootElement", "$collectionService", function($scope, $itemService, $collectionTypes, $mdDialog, $rootElement, $collectionService){
 		"ngInject";
